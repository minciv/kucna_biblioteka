...
# @Верзија  : 0.2.0
# @Програм  : Windsurf
# @Опис     : Датотека за упознавање са програмом Кућна Библиотека


# Кућна библиотека (Home Library) (v0.2.0) - Модернизована верзија
...
## 🆕 Нове функције у v0.2.0
- **Модернизовани модели података** са Pydantic валидацијом
- **Побољшано логовање** са Rich библиотеком и боље форматирање
- **Аутоматизовани тестови** са pytest framework-ом
- **Модерно пакетовање** са pyproject.toml и build системом
- **CLI интерфејс** са аргументима за напредне кориснике
- **Развојни алати** (black, ruff, mypy) за квалитет кода
- **Makefile** за лако управљање развојним задацима
...
### 🚀 Брзо покретање (Quick Start)
```bash
# Клонирај репозиторијум
git clone https://github.com/minciv/kucna-biblioteka.git
cd kucna-biblioteka
...
# Покрени апликацију
make run
# или python kucna_biblioteka.py
```

### 🛠️ Развојни режим (Development Mode)
```bash
# Инсталирај развојне зависности
make dev-install

# Покрени тестове
make test

# Провери квалитет кода
make lint

# Форматирај код
make format

# Покрени у debug режиму
make debug
```
...
# Са другачијим фајлом базе
python kucna_biblioteka.py --db mojabaza.csv

# Debug режим
python kucna_biblioteka.py --debug

# Прикажи верзију
python kucna_biblioteka.py --version

# Помоћ
python kucna_biblioteka.py --help
```
